...
User-facing strings live in `src/locales/`. When adding or changing strings:

- **Edit `src/locales/en.json`** (the English source file) and carry the
  same key changes into the other locale files you can translate. Other
  languages are community-maintained directly in this repo.
- Use `{{placeholder}}` syntax (double braces) for dynamic values, e.g.
  `"Imported {{count}} sessions"`.
- Run `yarn l10n:validate` to check that your JSON is valid and placeholders
  are consistent.
